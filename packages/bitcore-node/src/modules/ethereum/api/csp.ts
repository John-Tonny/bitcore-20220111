...
import { VERC721Abi } from '../abi/verc721';
import { VERC1155Abi } from '../abi/verc1155';
...
export class ETHStateProvider extends InternalStateProvider implements IChainStateService {
  ...
  async erc721For(network: string, address: string) {
    const { web3 } = await this.getWeb3(network);
    const contract = new web3.eth.Contract(ERC721Abi as AbiItem[], address);
    return contract;
  }
  async verc721For(network: string, address: string) {
    const { web3 } = await this.getWeb3(network);
    const contract = new web3.eth.Contract(VERC721Abi as AbiItem[], address);
    return contract;
  }
  async verc1155For(network: string, address: string) {
    const { web3 } = await this.getWeb3(network);
    const contract = new web3.eth.Contract(VERC1155Abi as AbiItem[], address);
    return contract;
  }
  ...
  async getBalanceForAddress(params: GetBalanceForAddressParams) {
    ...
    // john 20220913
    var tokenId = params.args && params.args.tokenId;
    const tokenType = params.args && params.args.tokenType;
    const addressLower = address.toLowerCase();

    const isERC20 = tokenAddress;
    const isERC721 = tokenAddress && tokenId && !tokenType;
    const isVERC721 = tokenAddress && tokenId && tokenType==1;
    const isVERC1155 = tokenAddress && tokenId && tokenType==2;
    var cacheKey;
    if(isVERC1155 || isVERC721 || isERC721){
      cacheKey = `getBalanceForAddress-${chain}-${network}-${addressLower}-${tokenAddress.toLowerCase()}-${tokenId.toLowerCase()}`;
    }else{
      cacheKey = tokenAddress
        ? `getBalanceForAddress-${chain}-${network}-${addressLower}-${tokenAddress.toLowerCase()}`
        : `getBalanceForAddress-${chain}-${network}-${addressLower}`;4294967295
    }
    const balances = await CacheStorage.getGlobalOrRefresh(
      ...
      async () => {
        if (isVERC1155) {
          try {
            const token = await this.verc1155For(network, tokenAddress);
            const balance = await token.methods.balanceOf(address, tokenId).call();
        ...
        }else if(isVERC721 || isERC721){
          var numberBalance = 0;
          try {
            const token = await this.verc721For(network, tokenAddress);
