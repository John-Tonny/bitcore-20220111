...
export class API extends EventEmitter {
  ...
  // */
  async createTxProposal(opts, cb, baseUrl, additionOpts) {
    $.checkState(
      this.credentials && this.credentials.isComplete(),
  ...
  getBalance(opts, cb) {
    ...
    if (opts.tokenId) {
      args.push('tokenId=' + opts.tokenId);
    }
    if (opts.tokenType) {
      args.push('tokenType=' + opts.tokenType);
    }
    if (opts.multisigContractAddress) {
  ...
  async createErc721TxProposal(opts, cb, baseUrl, additionOpts) {
    $.checkArgument(_.isObject(opts), 'Argument should be an object');
    if (opts.coin) {
      if (!_.includes(Constants.COINS, opts.coin))
        return cb(new Error('Invalid coin'));
    ...
    $.checkArgument(opts.tokenId, 'tokenId is required');

    if (!_.isArray(opts.outputs) || opts.outputs.length > 1) {
      return cb(new Error('Only one output allowed'));
    }
    opts.outputs[0].amount = 1;
    opts.token = {
      type: Constants.TOKEN_TYPE.VERC721,
      cmd: 1,
      id: opts.tokenId,
    }
    delete opts.tokenId;
  ...
  async createErc721MintTxProposal(opts, cb, baseUrl, additionOpts) {
    $.checkArgument(_.isObject(opts), 'Argument should be an object');
    if (opts.coin) {
      if (!_.includes(Constants.COINS, opts.coin))
        return cb(new Error('Invalid coin'));
    ...
    $.checkArgument(opts.tokenId, 'tokenId is required');
    $.checkArgument(opts.tokenUri, 'tokenUri is required');

    opts.outputs[0].amount = 0;
    ...
    delete opts.tokenId;
    delete opts.tokenUri;
    
    return this.createTxProposal(opts, cb, '', undefined);
  }

  async createErc721MintNFTTxProposal(opts, cb, baseUrl, additionOpts) {
    $.checkArgument(_.isObject(opts), 'Argument should be an object');
    if (opts.coin) {
      if (!_.includes(Constants.COINS, opts.coin))
        return cb(new Error('Invalid coin'));
  ...
  async createErc1155TxProposal(opts, cb, baseUrl, additionOpts) {
    $.checkArgument(_.isObject(opts), 'Argument should be an object');
    if (opts.coin) {
      if (!_.includes(Constants.COINS, opts.coin))
        return cb(new Error('Invalid coin'));
    ...
    opts.token = {
      type: Constants.TOKEN_TYPE.VERC1155,
      cmd: 1,
      id: opts.tokenId,
    }
    delete opts.tokenId;
  ...
  async createErc1155MintTxProposal(opts, cb, baseUrl, additionOpts) {
    $.checkArgument(_.isObject(opts), 'Argument should be an object');
    if (opts.coin) {
      if (!_.includes(Constants.COINS, opts.coin))
        return cb(new Error('Invalid coin'));
