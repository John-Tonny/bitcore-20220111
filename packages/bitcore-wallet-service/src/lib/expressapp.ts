...
*/
    ...
    router.get('/v1/balance/', (req, res) => {
      getServerWithAuth(req, res, server => {
        const opts: { coin?: string; twoStep?: boolean; tokenAddress?: string; multisigContractAddress?: string; tokenId?: number; tokenType?: number } = {};
        if (req.query.coin) opts.coin = req.query.coin;
        if (req.query.twoStep == '1') opts.twoStep = true;
        if (req.query.tokenAddress) opts.tokenAddress = req.query.tokenAddress;
        if (req.query.tokenId) opts.tokenId = req.query.tokenId;
        if (req.query.tokenType) opts.tokenType = req.query.tokenType;
