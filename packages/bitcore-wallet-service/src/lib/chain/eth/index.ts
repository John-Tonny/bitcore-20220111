...
export class EthChain implements IChain {
  ...
  getWalletBalance(server, wallet, opts, cb) {
    ...
    // john 20220913
    if(opts.tokenId){
      wallet.tokenId = opts.tokenId;
    }
    if(opts.tokenType){
      wallet.tokenType = opts.tokenType;
    }

    if(opts.tokenType && opts.tokenCmd
        &&  ((opts.tokenType == Constants.TOKEN_TYPE.VERC721 && (opts.tokenCmd ==2 || opts.tokenCmd == 3))
        || (opts.tokenType == Constants.TOKEN_TYPE.VERC1155 && opts.tokenCmd ==2))) {
      wallet.tokenAddress = undefined;
    }
            ...
            const byAddress = [
              {
                ...
                // tokenId: parseInt(opts.tokenId)  // john 20220913
              }
            ];
  ...
  getBitcoreTx(txp, opts = { signed: true }) {
    ...
    if (!txp.relay || !txp.relay.cmd) {
      if(!txp.token) {
        ...
        if(txp.token.type &&  txp.token.type == Constants.TOKEN_TYPE.VERC721){
          if(txp.token.cmd && txp.token.cmd == 1){
            chain = 'VERC721';
            txp.chain = chain;
            for (let index = 0; index < recipients.length; index++) {
              const rawTx = Transactions.get({ ...txp }).create({
                ...txp,
                ...recipients[index],
                chain,
            ...
            txp.chain = 'ETH';
          }else if(txp.token.cmd && txp.token.cmd == 2){
            chain = 'VERC721';
            txp.chain = chain;
            for (let index = 0; index < recipients.length; index++) {
              const rawTx = Transactions.get({ ...txp }).createMint({
                ...txp,
                ...recipients[index],
                chain,
                gasLimit: txp.gasLimit,
            ...
            txp.chain = 'ETH';
          }else if(txp.token.cmd && txp.token.cmd == 3) {
            chain = 'VERC721';
            txp.chain = chain;
            for (let index = 0; index < recipients.length; index++) {
              const rawTx = Transactions.get({ ...txp }).createMintNft({
                ...txp,
                ...recipients[index],
                chain,
                gasLimit: txp.gasLimit,
            ...
            throw new Error('token cmd is invalid');
          }
        }else if (txp.token.type && txp.token.type == Constants.TOKEN_TYPE.VERC1155){
          if(txp.token.cmd && txp.token.cmd == 1) {
            chain = 'VERC1155';
            txp.chain = chain;
            for (let index = 0; index < recipients.length; index++) {
  ...
  selectTxInputs(server, txp, wallet, opts, cb) {
    ...
    var tokenId, tokenType, tokenCmd;
    if(opts.token){
      tokenId = opts.token.id;
      tokenType = opts.token.type;
      tokenCmd = opts.token.cmd;
    }

    server.getBalance(
      { wallet, tokenAddress: opts.tokenAddress, multisigContractAddress: opts.multisigContractAddress, tokenId, tokenType, tokenCmd },
      (err, balance) => {
        if (err) return cb(err);
