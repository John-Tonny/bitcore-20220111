...
function v8network(bwsNetwork) {
  ...
  if (
    bwsNetwork == 'testnet' &&
    config.blockchainExplorerOpts.btc &&
...
export class V8 {
  ...
  async getBalance(wallet, cb) {
    const client = this._getAuthClient(wallet);
    const { tokenAddress, multisigContractAddress, tokenId, tokenType } = wallet;
    client
      .getBalance({ pubKey: wallet.beAuthPublicKey2, payload: {}, tokenAddress, multisigContractAddress, tokenId, tokenType })
      .then(ret => {
        return cb(null, ret);
      })
      .catch(cb);
